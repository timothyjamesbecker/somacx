...
des = """FASTQ Alignment Tool v0.1.2, 03/20/2020 Timothy James Becker"""
...
if args.fastq is not None:
    ...
    if len(files)<1:
        print('checking for master folder of sample folders..')
        files =  glob.glob(fastq_path+'/*/*.fq')+glob.glob(fastq_path+'/*/*.fq.gz')+\
                 glob.glob(fastq_path+'/*/*.fastq')+glob.glob(fastq_path+'/*/*.fastq.gz')
        if len(files)<1:
            ...
            fastq_paths = sorted(list(set(['/'.join(file.rsplit('/')[:-1])+'/' for file in files])))
    else:
...
if args.platform is not None:
    platforms = ['illumina','pacbio'] #can add more here
